...
//!
//! ```text
//! SYRINX_MODEL_DIR=~/.local/share/parakeet-dictation/nemotron \
...
fn model_dir() -> PathBuf {
    PathBuf::from(
        std::env::var("SYRINX_MODEL_DIR")
            .expect("set SYRINX_MODEL_DIR to the Nemotron model directory"),
    )
}
...
fn transcribes_golden_audio() {
    ...
    for m in session.push_audio(&audio).unwrap() {
        if let syrinx_proto::ServerMessage::TranscriptCommit { text: t, .. } = m {
            text.push_str(&t);
        }
    }
